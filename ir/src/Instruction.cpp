...
CmpInst::CmpInst(Type *ty, CmpOp op, OpID oid, Value *lhs, Value *rhs,
            BasicBlock *bb)
...
CmpInst *CmpInst::create_fcmp(CmpOp op, Value *lhs, Value *rhs, BasicBlock *bb)
{
    return new CmpInst(Type::get_int1_type(bb->get_module()), op, Instruction::fcmp, lhs, rhs, bb);
}

static std::string print_fcmp_type(CmpInst::CmpOp op) {
  switch (op) {
